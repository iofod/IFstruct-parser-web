...
/**
 *  {
 *    'width: 100%': [ hid|I, hid|I+, ...  ] I代表地几个状态  +代表active: true
 *    'height: 333px': [...]
 *  }
 *
 */

// px 为编辑器单位，可特殊处理化， 如 rem, vw, rpx...
// K 为提取为类的属性过滤， 比如 sh k=1 中 [xx].length = 1 & < 1 的过滤
function genStyle(ctree, type = 'px', K = 1) {
  ...
  for (let aid in SH) {
    ...
    // if (aid.includes('flex')) {
    //   console.log(aid, avs)
    // }
    // 注意：映射属性也在这里处理 ，如 display: flex  => dff
    if (utilAttrMap[aid]) {
      avs.forEach(hid => {
        if (CTV[hid]) {
      ...
      if (avs.length > K) {
        ...
        avs.forEach(id => {
          // let cv = HS[id].splice(HS[id].indexOf(aid), 1) 这种循环会有遗漏
  ...
  // console.log(idTree['u1584068668841|0'])
  // console.log(HS['u1584068668841|0'])
...
function floatSides(calc, key, unit = '') {
	if (typeof calc[key] == 'string') {
		let CV = calc[key].split(' ').map(v => str2num(v) + unit)
...
function px2any(obj, unit = 'px') {
  ...
  for (let key in obj) {
    ...
    if (useSides.includes(key)) {
      floatSides(obj, key, unit)
    } else {
      // 这里只处理非简写值，不适用与 padding margin borderRadius borderWidth
      if (/[0-9]px$/.test(V)) {
...
function getStatus(obj) {
  let nobj = {}
  Object.keys(obj).forEach(hid => {
    let target = obj[hid]

    nobj[hid] = target.status
  })
  return nobj
}
